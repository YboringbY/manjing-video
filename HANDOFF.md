...
- 新增纯增量 migration `20260714103000_add_material_content_hash`，只给 `Material` 增加 `contentHash / byteSize / mimeType` 和唯一索引，不删除、不回填业务行；已应用到本地开发库，尚未应用生产。
- 本地 TypeScript、生产构建、核心 API 集成均通过；5050 已重启为最终代码。生产部署前仍需单独批准迁移，并确认 Nginx `client_max_body_size` 与图片 25MB、音频 100MB、视频 500MB 的应用限制一致。
