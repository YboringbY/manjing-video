...
export type Project = { id: number; name: string; type: string; script: string; version?: number; updatedAt?: string; materialCount?: number };
export type Shot = { id: number; title: string; prompt: string; ratio: string; duration: number; status: ShotStatus; resolution?: "480p" | "720p" | "1080p"; width?: number; height?: number; version?: number; updatedAt?: string };
export type VideoTaskReference = { id: number | string; name: string; kind: MaterialKind | string };
export type VideoTaskSnapshot = { prompt: string; model?: string; ratio: string; duration: number; resolution?: "480p" | "720p" | "1080p"; materialIds: number[]; externalAssetIds: string[]; references?: VideoTaskReference[]; firstFrameMaterialId?: number; lastFrameMaterialId?: number; omniReferenceEnabled?: boolean; inputType?: "reference" | "first_last_frame" | "text_to_video" };
export type VideoTask = { id: string; shotId: number; shotTitle: string; provider: string; status: TaskStatus; result: string; providerTaskId?: string; apiProfile?: Pick<ApiProfile, "id">; snapshot?: VideoTaskSnapshot; videoUrl?: string; error?: string; rating?: "satisfied" | "unsatisfied"; feedback?: string; createdAt?: string; updatedAt?: string; completedAt?: string };
