...
- 当前守恒基线：项目 3、工作区 3、素材 16、素材关联 16、分镜 30、视频任务 26、视频资产 12、生图任务 2。
- 最近发布前后上述数量及主键指纹完全一致，没有执行恢复、清理、删除或回填。
- 团队共享素材数和项目素材数是不同口径；`teamMaterials=0` 不代表项目素材丢失。本次守恒确认素材与项目关联总数均为 16，单个项目数量必须按实时项目 API/UI 核对。
...
- 质量门禁现为非交互式 ESLint、TypeScript、生产构建、核心 API 集成、32 项单元测试和正式浏览器 smoke。

### 品牌版本（已发布生产）

- 工作区素材 `../源文件.pdf` / `../源文件.ai` 已满足正式替换需求：Illustrator 可编辑矢量、文字转曲、8 页彩色/单色组合，可机械提取路径且无字体依赖。
- 本地页面品牌已从“漫镜视频”替换为“果然漫镜”；新增展示版 Logo、侧栏紧凑 Logo 和 favicon 图形标。
- 品牌视觉规则：橙色 `#FFA131` 仅用于 Logo，不用于界面按钮或大面积背景；界面保持白/浅灰，主操作使用深蓝，蓝色 `#00A6FC` 仅用于链接、焦点和辅助提示，绿/红继续表达成功/失败。
- 登录页使用浅灰品牌区和白色登录区，不使用黑色/深色背景、橙色装饰边框或强阴影；工作台按钮、选中态和卡片强调保持深蓝/中性灰的克制层级。
- 桌面与 390×844 全新浏览器 smoke 均通过，Logo 清晰且无溢出，浅色登录页和按钮配色正常；移动端生成记录表格横向裁切作为独立响应式优化项保留。
- 本批只改前端和静态品牌资源，不涉及 API、数据库、migration、业务数据或 Nginx；已于 2026-07-24 发布生产 `8178020`，待执行 migration 为 0，业务数量和主键指纹守恒。
- 用户明确批准本次单次跳过登录态自动 smoke；未登录生产浏览器已验证浅灰登录页、深蓝按钮、Logo、favicon 和备案号。该例外不得自动延续到后续发布。
...
- 桌面仍使用原表格结构，数据、筛选、预览和操作行为不变；本地 ESLint、32 项单测、TypeScript、生产构建和全新 390×844 浏览器 smoke 已通过。
...
- 本批只改前端组件边界和 smoke 检查，不涉及 API、数据库、migration、业务数据或 Nginx；本地 ESLint、32 项单测、TypeScript、生产构建和全新浏览器 smoke 均通过，尚未发布生产。
...
- 本批只新增测试，不修改业务实现、API、数据库、migration、业务数据或 Nginx；ESLint、32 项单测、TypeScript 和生产构建均通过，尚未发布生产。

### 下一步优化顺序

1. 经用户确认后发布本地完成的生成记录移动端布局，并完成生产浏览器核验。
2. 经用户确认后发布本地完成的剧本工作台拆分，并完成生产浏览器核验。
3. 补做一次登录后的生产人工回归：侧栏 Logo、项目、素材、生成记录和任务反馈；本次自动登录态 smoke 已按用户批准跳过。
4. 后续单独评估 `npm ci` 报告的 3 个 high severity 依赖告警；先确认依赖链、可用修复版本和回归范围，不直接执行 `npm audit fix --force`，不阻断当前稳定版本。
5. 继续拆分分镜编辑/提示词拆分逻辑，保持 API、数据库与用户行为不变。
6. 对 `ProjectWorkspace.state` 做只读依赖清单和退役设计；稳定期内不直接删除字段、不做清理 migration。
7. 完成一次经用户确认的真实生产人工回归：素材上传/共享、真实视频生成、状态同步、预览下载和满意/需改进反馈；付费生成不放入自动 smoke。
8. 稳定版本确认后再把现有质量命令迁入 GitHub Actions；先做 CI，生产 CD 继续保留人工审批。
...
- 用户只批准部署代码修复，未批准 Nginx 配置调整或恢复 5 张历史孤立图片；本次严格按该范围执行。
- 生产从 `db77ddb` 更新到 `e49cc3c Persist generated images before responding`，migration 为 0。
- 部署前后业务数量与主键指纹一致：项目 1、工作区 1、素材 22、素材关联 22、分镜 27、任务 32、视频资产 10。
- 发布备份：`/data/backups/manjing-video-db-pre-deploy-20260713-223037.dump`，77KB、PostgreSQL custom archive、111 个目录项。
- 独立核验：生产 commit `e49cc3c`、工作区干净、PM2 online、公网首页 200、匿名鉴权 401。
- 当前 22 条 Material 全部为 `source=upload`；历史 5 张孤立生图仍只有文件和成功审计，没有恢复为 Material，符合批准范围。
- Nginx 仍使用默认代理超时；新代码保证应用完成后服务端自行落库，但浏览器仍可能先看到 504，需要稍后刷新素材库。调整代理超时仍需单独批准。
