...
- `app/page.tsx` 已从早期约 2890 行降到 2190 行；已拆出登录、人员、审计、项目列表、项目概览、生图、生成记录、素材库、剧本和分镜/提示词弹窗组件。
...
- 权威清单见 `docs/PROJECT_WORKSPACE_STATE_RETIREMENT.md`；已逐项覆盖工作区 API、前端 localStorage/自动同步、生产 smoke、核心集成测试和备份守恒脚本。
- `Project / Shot / VideoTask / VideoAsset / Material / ProjectMaterial / ImageTask` 是业务权威；`state` 只保留兼容元数据、乐观并发版本和旧响应形状，业务数组继续在写入时清空、读取时从规范化表水合。
- 退役分为冻结观察、最小元数据契约、迁移准备和受控退役四阶段。本批不改 schema、不新增 migration、不删除字段或业务行；设计文档不构成生产数据库变更批准。
