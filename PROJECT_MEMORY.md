...
- 本地品牌变更开始前的 GitHub `main` 基线为 `a3daae2 Refresh project progress and memory`；生产业务代码仍为 `c36cf6f`。
...
- 本批只改前端和静态品牌资源，不涉及 API、数据库、migration、业务数据或 Nginx；尚未发布生产，不能将本地品牌状态表述为线上已生效。

### 下一步优化顺序

1. 品牌版本经用户确认后，按生产安全流程独立发布并验证登录页、侧栏 Logo、favicon、备案号和真实项目页面。
2. 修复 390px 窄屏生成记录表格横向裁切，保证关键列和操作入口可访问。
3. 继续纯前端 P1：拆分剧本工作台和分镜编辑/提示词拆分逻辑，保持 API、数据库与用户行为不变。
4. 为 `lib/api-input.ts`、`lib/video-generation.ts`、`lib/video-status.ts` 补充独立单元测试。
5. 对 `ProjectWorkspace.state` 做只读依赖清单和退役设计；稳定期内不直接删除字段、不做清理 migration。
6. 完成一次经用户确认的真实生产人工回归：素材上传/共享、真实视频生成、状态同步、预览下载和满意/需改进反馈；付费生成不放入自动 smoke。
7. 稳定版本确认后再把现有质量命令迁入 GitHub Actions；先做 CI，生产 CD 继续保留人工审批。
